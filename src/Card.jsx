...
function Card({imgList}){
    ...
    const submitForm = (arr) => {
        if(arr.title.toLowerCase() === newImgList[randNum].title.toLowerCase() && arr.artist.toLowerCase() === newImgList[randNum].artist.toLowerCase()){
            alert('Congrats! Correct answer!');
            newImgList.filter(itm => newImgList[itm] !== newImgList[randNum])
            setDataArr(prev => prev - 1);
            setScore(prev + 1);
   
        } else if(arr.title.toLowerCase() === newImgList[randNum].title.toLowerCase() && arr.artist.toLowerCase() === newImgList[randNum].originalLetters) {
            alert('Congrats! Correct answer!');
            newImgList.filter(itm => newImgList[itm] !== newImgList[randNum]);
            setDataArr(prev => prev - 1);
            setScore(prev + 1);
        }else {
            alert(`The correct answer was "${newImgList[randNum].title}" by ${newImgList[randNum].artist}. :(`);
        }
    }
